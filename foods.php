...
<body>
	...
	if (isset($_GET['search'])) {
		...
		$query = $db->connect();
		try {
			$statement = $query->prepare("SELECT * from foods WHERE name LIKE :searchedFor");		
			$searched_for = '%'.htmlspecialchars($_GET['search']).'%';
			$statement->bindValue(':searchedFor', $searched_for, PDO::PARAM_STR);
			$statement->execute();
					...
					<?php for($e = 0; $e < sizeof($row); $e++) { ?>
						<div class="max-w-sm rounded overflow-hidden shadow-lg mx-5">
							...
							<div class="mx-auto text-center mx-7 my-4">
								<a href="<?php echo 'foods/order.php?id='.$row[$e]['id']; ?>" class="btn mr-2 rounded-full" style="background-color: crimson; font-family: 'Playfair Display'; font-weight: bolder;">
									Order
								</a>
							</div>
						</div>
